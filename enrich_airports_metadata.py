"""
Script pour enrichir la table airports avec les métadonnées d'OurAirports.
Ajoute airport_type et scheduled_service pour filtrer les aéroports commerciaux.
"""
...
def download_ourairports():
    ...
    try:
        ...
        logger.info(f"Downloaded {len(df)} airports from OurAirports")

        # Garder seulement les colonnes nécessaires
        df = df[['iata_code', 'type', 'scheduled_service']].copy()

        # Filtrer les aéroports avec code IATA
        df = df[df['iata_code'].notna()].copy()
        df = df[df['iata_code'].str.len() == 3].copy()

        logger.info(f"Found {len(df)} airports with valid IATA codes")
...
def update_airport_metadata(conn, ourairports_df):
    ...
    try:
        ...
        for _, row in ourairports_df.iterrows():
            ...
            scheduled = row['scheduled_service']
...
def main():
    """Script principal."""
    logger.info("=" * 80)
    logger.info("AIRPORT METADATA ENRICHMENT")
    logger.info("=" * 80)
    logger.info("")
    ...
    try:
        ...
        ourairports_df = download_ourairports()
        ...
        update_airport_metadata(conn, ourairports_df)
